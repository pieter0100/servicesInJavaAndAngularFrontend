...
public class DepartmentControllerImpl implements DepartmentController {
    ...
    @Autowired
    public DepartmentControllerImpl(DepartmentService departmentService, RestTemplate restTemplate) {
        ...
        DepartmentsResponse res = DepartmentsResponse.builder()
                .departments(deps.stream().map(departmentEntity -> DepartmentResponse.builder()
                            ...
                            .departmentName(departmentEntity.getDepartmentName())
                                .departmentBuildingNumber(departmentEntity.getDepartmentBuildingNumber())
                            .build())
                        .toList()
                )
                .build();
    ...
    public ResponseEntity<Void> deleteDepartment(UUID id) {
        ...
        String url = "http://backend-scientist:8082/departments/" + id;
    ...
    public void updateDepartment(UUID id, CreateDepartmentRequest dep) {
        ...
        //wyslij do serwisu drugiego
        // zapisz w serwisie scientis
        String url = "http://backend-scientist:8082/departments/" + id;

        try {
            // Wysyłamy ten sam obiekt request do drugiego serwisu
            restTemplate.put(url, dep, Void.class);
        } catch (Exception e) {
            // W labach wystarczy printstacktrace, w produkcji logujemy błąd,
            // że spójność danych została naruszona
            System.err.println("Błąd komunikacji z scientist-service: " + e.getMessage());
